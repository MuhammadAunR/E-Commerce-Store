...
const ProductCard = ({ product }) => {
    const imageSrc = product?.image?.[0]?.url
            ...
            <div key={product.id} className="card shadow-xl rounded-xl">
                <Link href={`/product/${product.slug}`}>
                    <div className=' w-[250px] h-80 rounded-xl relative overflow-hidden'>
                        <Image src={imageSrc} alt={product.title} fill unoptimized className='hover:scale-105 transition-transform ease-in-out duration-500 object-cover' />
                    </div>
                </Link>
